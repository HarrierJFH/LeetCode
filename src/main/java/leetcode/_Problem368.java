...
public class _Problem368 {
    ...
    public List<Integer> largestDivisibleSubset(int[] nums) {
        ...
        List<Node> nodes = new ArrayList<>(node.next);
        for (int i = 0; i < nodes.size(); i++) {
            node = nodes.get(i);
            nodes.addAll(node.next);
        }
    ...
    private void add(Node node, int num) {
        ...
        for (Iterator<Node> iterator = nodes.iterator(); iterator.hasNext(); ) {
            ...
            if (num < child.val && child.val % num == 0) {
                b = false;
                newNode.next.add(child);
                ...
                newNode.parent = node;
                break;
            } else if (num > child.val && num % child.val == 0) {
                b = false;
                add(child, num);
            }
        }
